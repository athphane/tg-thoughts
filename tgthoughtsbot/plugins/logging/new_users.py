...
@TgThoughtsBot.on_message(~custom_filters.group & custom_filters.private, group=-10)
async def creates_the_user(bot: TgThoughtsBot, message: Message):
    ...
    if user is None:
        ...
        if message.from_user.last_name and message.from_user.username:
            full_message = (
                f"**A new user has started using {str(TgThoughtsBot)}**\n"
...
@TgThoughtsBot.on_inline_query(group=9)
